...
print()
print("=" * 70)
print("LOADING TRAINED OCEANEMBED MODEL")
print("=" * 70)
...
validation_temperature = temperature.sel(
    time=slice(
        "2024-01-01",
        "2024-12-31"
    )
...
if len(validation_temperature.time) == 0:
...
validation_dates = [
    ...
    date(VALIDATION_YEAR, 1, 1)
    + timedelta(days=i)

    for i in range(
        len(validation_temperature.time)
    )
]
...
print(
    str(
        validation_temperature.time.values[0]
    )
)
...
print(
    str(
        validation_temperature.time.values[-1]
    )
)
...
while start_index < len(validation_dates):
...
print(
    "OCEANEMBED 2024 VALIDATION COMPLETE"
)
